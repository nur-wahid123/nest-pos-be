...
export class ProductRepository extends Repository<Product> {
    ...
    async init() {
        ...
        try {
            ...
            for (let index = 0; index < products.length; index++) {
                ...
                const existedProduct = await queryRunner.manager.findOne(Product, { where: { code: product.sku } })
                if (existedProduct) {
                    continue
                }
                ...
                let existedBrand = await queryRunner.manager.findOne(Brand, { where: { code: product.brand } })
                if (!existedBrand) {
                ...
                let existedCategory = await queryRunner.manager
                    .findOne(Category, { where: { code: product.category } })
                if (!existedCategory) {
                ...
                let existedUom = await queryRunner.manager
                    .findOne(Uom, { where: { name: product.unit } })
                if (!existedUom) {
                    existedUom = new Uom()
    ...
    applyFilters(qb: SelectQueryBuilder<Product>, query: QueryProductListDto) {
        ...
        if (code) {
            qb.andWhere('lower(product.code) like lower(:code)', { code: `%${code}%` })
        }
    }
}
