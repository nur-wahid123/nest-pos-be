...
export class PaymentRepository extends Repository<Payment> {
   ...
   */
  async paySale(paymentDto: CreateSalePaymentDto, userId: number) {
    ...
    try {
      ...
      if (Number(needToPay.needToPay) === Number(paid)) {
        ...
        if (sale.payments.length === 0) {
          await this.substractProductToInventory(
            queryRunner.manager,
            sale.saleItems,
            userId,
          );
      ...
      payment.code = await autoGenerateCodeBank(
        new Date(),
        queryRunner.manager,
        'PY',
      );
  ...
  async payPurchase(paymentDto: CreatePaymentDto, userId: number) {
    const { paid, purchaseCode, note } = paymentDto;
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    if (paid <= 0)
      throw new BadRequestException('The payment amount must not be zero');
    ...
    if (Number(paid) > Number(needToPay))
      throw new BadRequestException(
        'The payment amount must not exceed the amount due',
      );
    try {
      ...
      if (Number(needToPay) === Number(paid)) {
        ...
        if (purchase.payments.length === 0) {
          await this.addProductToInventory(
            queryRunner.manager,
  ...
  async calculateSaleNeedToPay(
    ...
    if (sale.dueDate) {
      if (sale.payments.length > 0) {
        ...
        lateFees = await this.checkLateFees(
          new Date(date),
   ...
   */
  async substractProductToInventory(
  ...
  async addProductToInventory(
    ...
    purchaseItems.map(async (v) => {
      ...
      if (!inventory) {
        inventory = ettManager.create(Inventory, {
        ...
        const inventoryLedger = ettManager.create(InventoryLedger, {
          ...
          inventory: inventory,
          qty: v?.qty,
          qtyBeforeUpdate: inventory.qty,
          qtyAfterUpdate: Number(v?.qty) + Number(inventory.qty),
          direction: 1,
          createdBy: userId,
        });
        inventory.qty = Number(inventory.qty) + Number(v?.qty);
