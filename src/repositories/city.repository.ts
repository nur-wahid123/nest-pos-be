    ...
    async initCreate(cities: InitCityDto[]): Promise<City[]> {
        ...
        try {
            ...
            for (let index = 0; index < cities.length; index++) {
                ...
                let currProvince = await queryRunner.manager.findOne(Province, { where: { name: province } })
                if (!currProvince) {
                    let currIsland = await queryRunner.manager.findOne(Island, { where: { name: island } })
                    if (!currIsland) {
